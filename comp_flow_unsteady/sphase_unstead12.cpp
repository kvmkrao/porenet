...
//	      else if(abs(sat[i]) >= sthres[i]) { //    xc[i]<= disp[i] && disp[i] <= xright) {
        	  ...
        	  //rho  = pressures[i]/(8.31447*298.15)*0.0291;  //kg/mol
      	          ...
      	          if(wet[i] != 1) {
      		      // gas sorption - modification to effective pore radius 
      		      //effr = 2.0*0.4*0.1*pressures[i]*pnond/(1+0.1*pressures[i]*pnond); 
      		      effr = 2.0*0.4*0.1*pressures[i]*pnond/(1+0.1*pressures[i]*pnond); 
                      ped  = nd[i] - effr; 
      		      //  cout << "effective radius" << effr <<"\t" << nd[i] << endl;  
                      //  cout << i << "radius" << nd[i] <<"\t "<< ped << endl;  
                      tmps = dv_sorption(ped, cnstc, rho/pnond);
      	              //tmps  = 6.0*pow(ped,2)*2.0e-7*cnstc/rho;
             	      //cout << " sorption " << i <<"value "<< tmps/dt <<" "<< effr <<"\t"<< nd[i] <<"\t "<< ped << endl;   
             	      cout << " sorption " << i <<"value "<< rho <<" "<< effr <<"\t"<< nd[i] <<"\t "<< ped << endl;   
      	          }
     ...
     */
                  ...
                  if(wet[i] != 1) rhs[i] = rhs[i]-pressures[i]*(tmps/dt); // - rpart1/dt ;
                  row.push_back(i);
                  col.push_back(i);
