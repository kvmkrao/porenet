      ...
      for(int k=1; k<nsteps; k++) {
		     ...
		     sumcond  = 0;
			     ...
			     for(int j=0; j<ni[i]; j++) {
				     ...
				     k2  = l2[id];     // node on the other end of the link 
				     {
					     ...
					     //only pore throats offer resistance to flow but they do not have volume 
					     ...
					     tmpl  = conductance(ld[id], mu,linkl)*((pressures[k1]+pressures[k2])/(2.0*pressures[i])) ; // compressible 
					     tmp   = tmpl; //1.0/(1.0/tmpi + 1.0/tmpl + 1.0/tmpj);
			     ...
			     val.push_back(-sumcond); // add compressibility term 
	     ...
	     for(int i=0; i<nsec; i++) {
		     for(int j=0; j<ni[i]; j++) {
			     ...
			     if(i==k1) flowr =  flowr  + tmp * (pressures[k1] - pressures[k2]);
			     if(i==k2) massin = massin + tmp * den[i]*(pressures[k2] - pressures[k1]);
			     if(i==k2) flowr  = flowr  + tmp * (pressures[k2] - pressures[k1]);
