                        ...
                        int &iters)
{
   ...
   PetscReal  norm = 0, tol = 1.0e-20;     /* norm of solution error */
   ...
   ierr = PetscMalloc1(num_row+1, &d_nnz);CHKERRQ(ierr);
   ierr = PetscMalloc1(num_row+1, &o_nnz);CHKERRQ(ierr);
   std::vector<int> off_nnz_pet;  
   printf("num row col %d %d %d\n", num_row, num_col, row.size());

   for(int i=0; i <elementCount; i++)  {
	   d_nnz[i] = dia_nnz_pet[i]; 
	   o_nnz[i] = 1; 
	   //off_nnz_pet[i] = dia_nnz_pet[i]; 
	   //cout << i << "\t" << dia_nnz_pet[i] << endl; 
   }
   //exit(0); 
   //return 0;  
...
//    ierr = MatSetSizes(A,PETSC_DECIDE,PETSC_DECIDE,elementCount,elementCount); CHKERRQ(ierr);
   ...
   //ierr = MatMPIAIJSetPreallocation(A,maxd_nz,NULL,maxd_nz,NULL); CHKERRQ(ierr);
   //ierr = MatMPIAIJSetPreallocation(A,NULL,&dia_nnz_pet[0],NULL,&off_nnz_pet[0]); CHKERRQ(ierr);
//   ierr = MatSeqAIJSetPreallocation(A, NULL, d_nnz);
   ierr = MatMPIAIJSetPreallocation(A,NULL,d_nnz,NULL,o_nnz); CHKERRQ(ierr);
...
//    if (debug){
//        PetscViewer viewer;
//        PetscViewerASCIIOpen(PETSC_COMM_WORLD,"rhs_pet.dat", &viewer);
//        PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_PYTHON);
//        VecView(rhs_pet,viewer);
//        PetscViewerPopFormat(viewer);
//        PetscViewerDestroy(&viewer);

//        PetscViewerASCIIOpen(PETSC_COMM_WORLD,"A_mat.dat", &viewer);
//        PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_PYTHON);
//        MatView(A,viewer);
//        PetscViewerPopFormat(viewer);
//        PetscViewerDestroy(&viewer);

//        PetscViewerASCIIOpen(PETSC_COMM_WORLD,"x_pet.dat", &viewer);
//        PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_PYTHON);
//        VecView(x_pet,viewer);
//        PetscViewerPopFormat(viewer);
//        PetscViewerDestroy(&viewer);
//    }
    ...
    */
    cout << " residue iter " << resid <<" "<<its << endl; 
    for (auto i=0;i<pressures.size();i++) {	    
        pressures[i] = x_sol[i];
