...
*/
      ...
      for(int k=1; k<nsteps; k++) {
			     ...
			     for(int j=0; j<ni[i]; j++) {
				     ...
				     if(k1 == 0 && i == k2-1){      // check whether it is connected to inlet/outlet 
					     if(lid[id] == 1) { //  connected to inlet 
						     ...
						     effr  = 0.0; 
						     tmpl  = conductance(ld[id], mu,linkl)*((pinlet+pressures[i])/(2.0*pressures[i])) ; // compressible 
						     tmp   = tmpl*dt;  
						     ...
						     linkl = ll[id]; // - nd[k2-1]; // link length 
						     effr  = 0.0; 
						     tmpl  = conductance(ld[id], mu,linkl)*((poutlet+pressures[i])/(2.0*pressures[i])) ; // compressible 
						     tmp   = tmpl * dt;
					     ...
					     effr  = 0.0; //0.4*0.1*pressures[i]/(1+0.1*pressures[i]); // pressure in MPa
					     tmpl  = conductance(ld[id], mu,linkl)*((pressures[k1-1]+pressures[k2-1])/(2.0*pressures[i])) ; // compressible 
	     ...
	     for(int i=0; i<nodes; i++) {
		     ...
		     den[i]  = tmp*(abs(flrt[i])*pnond + voli)/ (voli) ;
		     //cout << i <<"\t" << tmp <<"\t" << flrt[i] <<"\t"<< den[i] << endl; 
	     ...
	     for(int i=0; i<links; i++) {
		     ...
		     if(k1 == 0) { 
			     if(lid[id] == 1) {   // inlet 
				     ...
				     tmpl    = conductance(ld[id]-effr, mu,linkl)*((pinlet+pressures[k2-1])/(2.0*pressures[k2-1])) ; // compressible 
				     tmp     = tmpl*dt; //1.0/(1.0/tmpi + 1.0/tmpl + 1.0/tmpj);
				     ...
				     flowr   = flowr  + tmpl*(pinlet - pressures[k2-1]);
		                     //cout << "mass in " <<"\t" << tmp <<"\t" << density <<"\t"<< pressures[k2-1] <<"\t"<< pinlet << endl; 
				     ...
				     flowr   = flowr   + tmpl*(pressures[k2-1] - poutlet);
		                     //cout << "mass out " <<"\t" << tmp <<"\t" << density <<"\t"<< pressures[k2-1] <<"\t" <<poutlet << endl; 
			     }
		     }
	     }
